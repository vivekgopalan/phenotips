...
public class DefaultGenePanelsLiveTableResourceImpl extends XWikiResource implements GenePanelsLiveTableResource
{
    ...
    public Response getGeneCountsFromPhenotypes(
        ...
        final int reqNo)
    {
        final List<String> present = makeTermList(presentTerms);
        final List<String> absent = makeTermList(absentTerms);
        ...
        try {
            final JSONObject panelJSON = getPanelData(present, absent, offset, limit);
     ...
     *
     * @param present a list of term identifiers observed to be present
     * @param absent a list of term identifiers observed to be absent
    ...
    private JSONObject getPanelData(
        @Nonnull final List<String> present,
        @Nonnull final List<String> absent,
        final int offset,
        final int limit) throws ExecutionException, IndexOutOfBoundsException
    {
        final GenePanel panel = this.genePanelLoader.get(present);
    ...
    /**
     * Makes an unmodifiable list of terms specified in {@code terms}, with any null values filtered out.
     *
     * @param terms a nullable list of terms, that may contain null values
     * @return an unmodifiable list of terms that does not contain any null values
     */
    private List<String> makeTermList(@Nullable final List<String> terms)
    {
        final List<String> nullFiltered = Optional.ofNullable(terms)
            .orElseGet(Collections::emptyList)
            .stream()
            .filter(Objects::nonNull)
            .collect(Collectors.toList());
        return Collections.unmodifiableList(nullFiltered);
    }
}
