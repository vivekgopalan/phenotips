...
public class DefaultGenePanelsLiveTableResourceImplTest
{
    ...
    public void setUp() throws Exception
    {
        ...
        when(this.panelLoader.get(anyListOf(String.class))).thenReturn(this.panel);
    ...
    public void getGeneCountsFromPhenotypesPresentAndAbsentTermsEmptyResultsInNoContentResponse()
    {
        ...
        final Response response = this.component.getGeneCountsFromPhenotypes(rawPresentTerms, rawAbsentTerms, 1, LIMIT,
            REQ_NO);
    ...
    public void getGeneCountsFromPhenotypesOffsetOutOfBoundsResultsInBadRequestResponse()
    {
        ...
        final int offset = 20;
        final Response response = this.component.getGeneCountsFromPhenotypes(rawPresentTerms, rawAbsentTerms, offset,
            LIMIT, REQ_NO);
    ...
    {
        when(this.panelLoader.get(anyListOf(String.class))).thenThrow(new ExecutionException(new Throwable()));

        final List<String> rawPresentTerms = Collections.emptyList();
        final List<String> rawAbsentTerms = Arrays.asList(HP1, HP2, HP3);
        final Response response = this.component.getGeneCountsFromPhenotypes(rawPresentTerms, rawAbsentTerms, 1, LIMIT,
            REQ_NO);
        verify(this.logger).warn("No content associated with [present-term: {}, absent-term: {}].", rawPresentTerms,
            rawAbsentTerms);
        Assert.assertEquals(Response.Status.NO_CONTENT.getStatusCode(), response.getStatus());
    }
    ...
    public void getGeneCountsFromPhenotypesPanelSmallerThanRequestedNumberOfTermsReturnsCorrectResponse()
    {
        ...
        final Response response = this.component.getGeneCountsFromPhenotypes(rawPresentTerms, rawAbsentTerms, 3, LIMIT,
            REQ_NO);
    ...
    {
        when(this.panelLoader.get(anyListOf(String.class))).thenThrow(new IllegalStateException());
        final List<String> rawPresentTerms = Arrays.asList(HP1, HP3, HP4, HP5, HP6);
        final List<String> rawAbsentTerms = Collections.singletonList(HP2);
        final Response response = this.component.getGeneCountsFromPhenotypes(rawPresentTerms, rawAbsentTerms, 3, LIMIT,
            REQ_NO);
